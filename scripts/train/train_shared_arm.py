...
class StatusCallback(BaseCallback):
    ...
    def _refresh_metric_cache(self):
        ...
        # (rollout/ep_rew_mean, rollout/ep_len_mean, time/fps are recorded and
        # dumped atomically inside SAC.dump_logs() with no callback in between, so
        # they never land here at all -- those are read from persistent model
        # state directly in _dump_status instead.)
